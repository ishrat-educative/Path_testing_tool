...
const saveToFile = (href, hrefTitle, incorrectURLs) => {
  console.log("save to File called.....");
  let found = false;
  let hrefsToSave = "";
...
const saveHrefsToFile = (hrefs) => {
  ...
  try {
    hrefs.forEach(async (element, index) => {
      ...
      try {
        ...
        console.log(
          "Error caught for stale element in getAttribute.. moving forward"
        );
      }
      console.log("Inside forEach...after href index :" + index);

      try {
        hrefTitle = await element.getText();
      } catch (error) {
        console.log(
          "Error caught for stale element..in getText() moving forward"
        );
      }

      console.log("Inside forEach...after getText() at index: " + index);

      incorrectURLs = await saveToFile(href, hrefTitle, incorrectURLs);

      console.log("Inside forEach...after saveToFile() at index: " + index);

      if (index == hrefs.length - 1) {
        console.log(
          "Inside forEach...calling loadNextlesson at index: " + index
        );

        setTimeout(loadNextLesson, 3000);
      }
  ...
  /*hrefs.forEach((element, index) => {
    ...
    if (index == hrefs.length - 1) {
      console.log("Called next lesson");
      loadNextLesson();
    }
  });*/
};
const parseHREFs = async () => {
  ...
  console.log("Inside Parse HREFS.....");
...
const setChromeWindow = (window_position) => {
  console.log("Inside setChromeWindow");
...
function loadNextLesson_sub() {
  console.log("Inside LoadNextLessson");
  driver.getCurrentUrl().then((url) => {
    console.log(
...
const getModuleEditorPage = () => {
  console.log("Inside getModuleEditorPage");
...
const loadNextModule = () => {
  console.log("Inside LoadNextModule");
  setChromeWindow(0);
  driver
    .wait(
      until.elementsLocated(
...
const getPathEditorPage = async () => {
  ...
  /*await driver.get(
    "https://www.educative.io/patheditor/10370001/6371598500626432"
  );*/

  /*await driver.get(
    "https://www.educative.io/patheditor/10370001/5741517777403904"
  );*/

  //Fundamentals of Web programming

  /*await driver.get(
    "https://www.educative.io/patheditor/10370001/5741699990552576"
  );*/
